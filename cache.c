...
void clear_cache_stats(struct cache_t *cp)
{
  ...
  cp->hits = 0;
  cp->misses = 0;
  cp->LRU_misses = 0; //LRU misses
  cp->BIP_misses = 0; //BIP missesf
  cp->PSEL = 0;	      //PSEL value
  cp->BIPCTR = 0; 
  cp->read_misses = 0;
  cp->write_misses = 0;
  cp->replacements = 0;
...
// Aris: only used for decay (leakage)
...
// -----------------------------------------------------------------------
// Aris: The purpose here is fast simulation while maintaining all
//  useful state. The code does not reflect how a cache is organised
//  in hardware!
// -----------------------------------------------------------------------
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  if (b_in_dispatch)  // Aris: doesn't do anything
	  b_in_dispatch = TRUE;
  ...
  if (CACHE_TAGSET(cp, addr) == cp->last_tagset) {
      ...
      blk = cp->last_blk;
      goto cache_fast_hit;  // Aris: Yes C has gotos!
  }
  ...
  if (cp->hsize) {
	...
	} else { /* low-associativity cache, linear search the way list */
  ...
  switch (cp->policy) {
  case LRU:// Aris : FIFO might look identical to LRU, but on access a block is 
              //  moved to the head of the list; this does not happen for FIFO.
	
    // Aris: The tail of the way-list holds the oldest (FIFO) or least used block (LRU)
...
#endif /* defined(cache_decay) */
	if(strcmp(cp->name, "dl1") == 0){ 					//CHECK CACHE LEVEL
		if(cp->nsets <= 64){                                         	// FULLY ASSOC OR SET ASSOC WITH LITTLE NSETS
			update_way_list(&cp->sets[set], repl, Head);         	//LRU
		}
		else{
			int BIP_distance = (cp->nsets / 32) - 1;
			int LRU_distance = (cp->nsets / 32) + 1;
			if((set % LRU_distance) == 0){               		//LRU
				update_way_list(&cp->sets[set], repl, Head);
				if(cp->LRU_misses < cp->nsets){
					cp->LRU_misses++;
				}
			}
			else if(set % BIP_distance == 0){	     		//BIP
				if(cp->BIP_misses < cp->nsets){
					cp->BIP_misses++;
				}
				cp->BIPCTR++;
				if(cp->BIPCTR == 32){
				...
				cp->PSEL = cp->LRU_misses - cp->BIP_misses;
				if(cp->PSEL < 0){
					update_way_list(&cp->sets[set], repl, Head);
				} else{
					cp->BIPCTR++;
	...
	break;
  case FIFO:  // Aris: FIFO might look identical to LRU, but on access a block is 
              //  moved to the head of the list; this does not happen for FIFO.
...
#endif /* defined(cache_decay) */
    // Aris: the block to be replaced will become the newest after replacement,
    //   so move to the front
    update_way_list(&cp->sets[set], repl, Head);
    break;
  case Random:
    {
  ...
  } // Aris: endswitch cache replacement policy
  ...
  cp->last_tagset = 0;  // Aris: isn't this a valid tag? Best set to -1
  cp->last_blk = NULL;

  /* Leakage: update stats */
  if (low_leak_penalty_flag == 1
    && /* repl-> status & CACHE_BLK_DECAYED */ cache_leak_is_ctrlled() ) {
  ...
  if (repl->status & CACHE_BLK_VALID) {
      cp->replacements++;
      if (repl_addr)  // Aris: caller asked to get address of replaced block
      ...
      if (repl->status & CACHE_BLK_DIRTY) {
	      ...
	      cp->writebacks++;
          // Aris: call the access function kept in the cache struct
          //  This is how cache misses are handled!
  ...
  /* read data block */ 
  // Aris: calls the cache's access function. This is the cache miss handler
  lat += cp->blk_access_fn(Read, CACHE_BADDR(cp, addr), cp->bsize, repl, now+lat);

  /* copy data out of cache block */
  // Aris: if set, the cache keeps the actual data and the macro does the
  //  data transfer in the direction needed (read,write)
  // Currently this is not used, data are found only in main memory
  //  and caches keep only tags, dirty, etc. 
  if (cp->balloc) {
  ...
  if (udata) // Aris: valid pointer only if requested
      // Aris: this is for meta data associated with a cache line
      *udata = repl->user_data;

  /* Leakage: for misses in low leak mode */  // Aris: stray comment
  ...
  // Aris: see detailed earlier comment 
  if (cp->balloc) {
  ...
  /* get user block data, if requested and it exists */
  // Aris: that's meta-data. Not used
  if (udata)
    *udata = blk->user_data;
